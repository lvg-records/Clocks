	...
	public class ClockPoint {
		...
		private static function tweenPoint():void {
			...
			if(lastUsedPoint > 0 && lastUsedPoint < 7) {
				...
				Tweener.removeAllTweens();
				Tweener.addTween(Clock.arrPoints[lastUsedPoint].point,
								 {alpha:Math.abs(Clock.arrPoints[lastUsedPoint].point.alpha - 1), time:curSpeed, onComplete:tweenPoint,
							transition:trnsation_type});
			}
		}
