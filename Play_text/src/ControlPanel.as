package  
{
	...
	public class ControlPanel extends Sprite
	{
		...
		private var btnChangeText	:Button;
		...
		public static var txtText			:TextInput;
		...
		private function init(e:Event=null):void 
		{
			...
			btnChangeText = preferancePanel['btn_element_12'];
			btnChangeText.addEventListener(MouseEvent.CLICK, changeText);
			
			txtText = preferancePanel['txt_element_12'];
		...
		private function changeText(e:MouseEvent = null ):void 
		{
			Clock.text = txtText.text;
		}
		...
		{
			Clock.t.delay = 1000/Number(txtSpeed.text);
		}
		
		private function changeColor(e:MouseEvent = null):void 
		{
			Clock.nuber_txt.txt.textColor = uint('0x' + txtElementColor.text);
			//for (var i:int = 0; i < Clock.arrClockElements.length; i++ )
			//{
				//if (Clock.arrClockElements[i] is Sprite)
				//{
			...
			//Clock.indicator.graphics.beginFill(uint('0x' + txtElementColor.text));
