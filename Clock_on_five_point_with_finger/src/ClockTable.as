package  
{
	...
	public class ClockTable 
	{
		...
		public static function lightClock(arr:Vector.<mcClockElement>, mode:int, date:Date ):void
		{
		...
		static private function showDefineSector(arr:Vector.<mcClockElement>,element_1:int = -1):void
		{
			...
			if (Clock.finger.q)
				Clock.finger.q.alpha = 0;
			
			var _delta:Number = 3;
			var tt:Number = Clock.t.delay *.5 - Clock.t.delay * .1;
			tt *= 0.001;
			//tt = 0.3;
			
			var isWasBack:Boolean = false;
			for (var j:int = 0; j < moveEl.length; j++)
			{
				Tweener.addTween(moveEl[j].bbb._press, { time:tt, y:moveEl[j].bbb._press.y + _delta, onComplete:back, transition:ClockTable.TYPE_0_TO_1 } );
				if (Clock.finger.q)
					Tweener.addTween(Clock.finger.q, { time:tt, alpha:1 } );
			}
			function back():void 
			{
				if (isWasBack)
				...
				for (var j:int = 0; j < moveEl.length; j++)
				{
					Tweener.addTween(moveEl[j].bbb._press, { time:tt, y:moveEl[j].bbb._press.y - _delta} );
					if (Clock.finger.q)
						Tweener.addTween(Clock.finger.q, { time:tt, alpha:0} );
				}
			}
		}
	}
